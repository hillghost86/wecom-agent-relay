...
/** 单聊发给管理员并等回执；name 是日志里的通知名，okLogArgs 是成功时 log 的参数（各自原有文字） */
async function notifyAdmin(conn, content, name, okLogArgs) {
  try {
    ...
    if (resp.errcode === 0) conn.bot.log(...okLogArgs);
    else conn.bot.warn(`${name}被拒 errcode=${resp.errcode} errmsg=${resp.errmsg}`);
  } catch (e) {
    conn.bot.warn(`${name}失败:`, e.message);
  }
}
...
export async function sendOutageReport(conn, { since, now, secs, reason }) {
  ...
  await notifyAdmin(conn, content, '断线自报', [`断线自报已发送：离线 ${secs}s（${reason}）`]);
}
...
export function startOfflineAlert(bot) {
  ...
  const tick = async () => {
    ...
    if (!content) return;
    await notifyAdmin(conn, content, '离线告警', ['离线告警已发送:', content]);
  };
