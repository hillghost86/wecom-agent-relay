...
启动时用 `--config <路径>` 指定，不给就读工作目录下的 `config.json`。缺必填项、`bots` 为空、key 不合法、token 或 `msg_log` 冲突都会打印原因后退出。模板在 `server/config.example.json`。**文件里有 Secret 和 token，`chmod 600`，绝不入库。**
...
| 项 | 必填 | 默认 | 说明 |
|---|---|---|---|
| `key` | 单 bot 可省 | `default` | 机器人标识，小写字母 / 数字 / 下划线 / 减号。`/health` 的 `bot` 字段和接口前缀 `/bots/<key>/` 就是它 |
